...
test("본문 볼드 부족과 AI 상투어 반복을 찾는다", () => {
  ...
  assert.ok(issues.some((issue) => issue.startsWith("AI 상투 표현이 반복됩니다.")));
});
...
test("소제목 태그의 숫자는 수치로 잠그지 않는다", () => {
  /*
   * <h2> 의 2 를 마커로 바꾸면 태그 이름이 <hWOOLIMLOCKBODYAEND> 가 됩니다.
   * 정리기는 모르는 태그를 통째로 버리므로 마커가 사라지고, 되돌릴 때
   * "누락되거나 중복" 으로 그 구간을 전부 잃었습니다. 본문은 소제목 단위로
   * 잘라 맡기므로 거의 모든 구간이 <h2> 로 시작합니다.
   */
  const source = "<h2>제안 배경</h2><h3>세부</h3><p>계약은 12개월입니다.</p>";
  const locked = lockValue(source, "BODY", true);
  assert.ok(locked.value.includes("<h2>"), "소제목 태그가 깨졌습니다.");
  assert.ok(locked.value.includes("<h3>"), "소제목 태그가 깨졌습니다.");
  assert.ok(!locked.value.includes("12개월"), "사람이 읽는 수치는 잠겨야 합니다.");
  assert.equal(restoreLocked(locked.value, locked.locks), source);
});
