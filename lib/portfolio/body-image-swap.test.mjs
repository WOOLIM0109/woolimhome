...
test("새 그림이 더 많으면 본문을 지우지 않고 부족한 자리를 만든다", () => {
  const result = swapPortfolioBodyImages(본문(3), [
    "/new/a.jpg",
    "/new/b.jpg",
    "/new/c.jpg",
    { url: "/new/d.jpg", caption: "네 번째 목업" },
  ]);
  assert.deepEqual(그림주소(result.bodyHtml), [
    "/new/a.jpg",
    "/new/b.jpg",
    "/new/c.jpg",
    "/new/d.jpg",
  ...
  assert.ok(result.bodyHtml.includes("소제목 1"));
  assert.ok(result.bodyHtml.includes("소제목 2"));
  assert.ok(result.bodyHtml.includes("소제목 3"));
  assert.ok(result.bodyHtml.includes("네 번째 목업"));
  assert.ok(!/<\/figure>\s*<figure\b/i.test(result.bodyHtml));
});
