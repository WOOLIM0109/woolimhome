...
test("예산 상한 안내는 수정 요청으로 취급하지 않는다", () => {
  // 실제로 보고된 현상: 상한 안내가 "보존된 수정 요청" 상자에 그대로 남아
  // 다음 생성 때 수정 지시로 AI에게 전달됐습니다.
  assert.equal(
    editorialRevisionNote("일일 Gemini 비용 상한을 초과합니다. (이번 달 사용 12회 / 상한 500회)"),
    null,
  );
  assert.equal(editorialRevisionNote("월간 Gemini 호출 상한을 초과합니다."), null);
  assert.equal(editorialRevisionNote("GEMINI_BUDGET_EXCEEDED"), null);
  assert.equal(
    pendingRevisionNote({ pendingRevision: { note: "일일 Gemini 비용 상한을 초과합니다." } }),
    null,
  );
});

test("사람이 남긴 수정 요청은 그대로 살린다", () => {
  assert.equal(
    editorialRevisionNote("첫 문단을 더 쉽게 고쳐 주세요."),
    "첫 문단을 더 쉽게 고쳐 주세요.",
  );
});
