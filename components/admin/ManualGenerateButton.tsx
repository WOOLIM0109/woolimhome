...
export default function ManualGenerateButton() {
  ...
  return <div><button type="button" disabled={loading} onClick={() => void generate()} className="inline-flex items-center gap-2 rounded-xl border border-[var(--line)] bg-white px-4 py-3 font-bold disabled:opacity-50"><Bot size={17} /> {loading ? "초안 생성 중…" : "디자인 인사이트 시험 초안"}</button>{message && <p className="mt-2 max-w-sm text-sm font-bold text-[var(--primary)]">{message}</p>}</div>;
}
