...
test("runtime activation warns about privacy terms without blocking", async () => {
  // 이름 목록이 비어 있어도 자동화는 돌아가되, 경고는 남깁니다.
  await withEnvironment({
    GEMINI_ENABLED: "true",
    ...
    assert.match(status.warning, /PII_REDACTION_TERMS/);
  });
  // 예전처럼 필수로 되돌리고 싶을 때를 위한 스위치.
  await withEnvironment({
    GEMINI_ENABLED: "true",
    GEMINI_API_KEY: "stub-only",
    NODE_ENV: "production",
...
test("호출 횟수 상한은 제공자를 부르기 전에 막는다", () => {
  const config = {
    dailyCalls: 1,
    monthlyCalls: 2,
    dailyCostUsd: 1,
  ...
  // 작업 하나가 상한보다 큽니다. 기다려도 실행되지 않습니다.
  assert.equal(budgetDecision({
    dailyCallsUsed: 0,
    monthlyCallsUsed: 0,
    dailyCostUsed: 0,
...
test("비용만 넘긴 경우에는 막지 않고 알려 준다", () => {
  /*
   * 예전에는 이 경우도 막았습니다. 그런데 여기 쓰이는 단가는 실제 값이 아니라
   * 크게 잡은 추정치입니다. 실제로 나간 돈보다 한참 앞서 닫히고, 화면에는
   * 아직 여유가 있는 호출 횟수가 함께 나와 이유를 알 수 없었습니다.
   * 사실이 아닌 이유로 잠그면 지키는 것이 아니라 방해하는 것이 됩니다.
   */
  const config = {
    dailyCalls: 1,
    monthlyCalls: 2,
...
/**
 * 칼럼 한 편이 6회를 예약하는데 일일 상한이 3회였습니다. 예산이 모자란
 * 것이 아니라 한 번도 시작할 수 없는 설정이어서, 칼럼 기능이 조용히
 * 멈춰 있었습니다. 같은 일이 다시 생기면 여기서 걸립니다.
 */
test("작업 하나가 상한보다 크면 예산이 남아 있어도 막고, 그 사실을 알린다", () => {
  const verdict = budgetDecision(
    { dailyCallsUsed: 0, monthlyCallsUsed: 0, dailyCostUsed: 0, monthlyCostUsed: 0 },
    0,
    { dailyCalls: 3, monthlyCalls: 30, dailyCostUsd: 100, monthlyCostUsd: 100 },
    6,
  );
  assert.equal(verdict.allowed, false);
  assert.match(verdict.reason, /상한을 올리기 전에는 실행되지 않습니다/);
});

test("기본 상한은 가장 큰 작업 하나는 담을 수 있어야 한다", () => {
  // 칼럼 생성·주제 교체·본문 재생성이 모두 6회를 예약합니다.
  const config = geminiBudgetConfig();
  assert.ok(config.dailyCalls >= 6, `일일 기본 상한이 ${config.dailyCalls}회라 칼럼을 시작할 수 없습니다`);
  assert.ok(config.monthlyCalls >= 6, `월간 기본 상한이 ${config.monthlyCalls}회입니다`);
});

test("막힌 이유에는 그 항목의 숫자가 함께 나온다", () => {
  const config = { dailyCalls: 20, monthlyCalls: 500, dailyCostUsd: 1, monthlyCostUsd: 10 };

  // 비용에 걸렸는데 호출 횟수를 보여 주던 것이 문제였습니다.
  // 194회 / 상한 500회처럼 아직 여유가 있는 숫자가 나와서, 왜 막혔는지 알 수 없었습니다.
  const cost = budgetDecision(
    { dailyCallsUsed: 3, monthlyCallsUsed: 194, dailyCostUsed: 0.1, monthlyCostUsed: 9.99 },
    0.5,
    config,
  );
  // 비용 기준을 넘어도 막지 않고 알려만 줍니다. 실제로 지출을 묶는 것은
  // 호출 횟수 쪽이고, 여기 단가는 크게 잡은 추정치라 사실과 어긋납니다.
  assert.equal(cost.allowed, true);
  assert.match(cost.warning, /기준을 넘었습니다/);
  assert.match(cost.warning, /\$10\.00/);
  assert.match(cost.warning, /GEMINI_MONTHLY_COST_LIMIT_USD/);
  assert.doesNotMatch(cost.warning, /194/);

  const calls = budgetDecision(
    { dailyCallsUsed: 0, monthlyCallsUsed: 499, dailyCostUsed: 0, monthlyCostUsed: 0 },
    0.001,
    config,
    6,
  );
  assert.equal(calls.allowed, false);
  assert.match(calls.reason, /월간 Gemini 호출 상한/);
  assert.match(calls.detail, /499회 \+ 이번 작업 6회 \/ 상한 500회/);

  const fine = budgetDecision(
    { dailyCallsUsed: 0, monthlyCallsUsed: 0, dailyCostUsed: 0, monthlyCostUsed: 0 },
    0.001,
    config,
  );
  assert.equal(fine.allowed, true);
  assert.equal(fine.detail, null);
  assert.equal(fine.warning, undefined);
});

test("비용 기준을 넘어도 호출 횟수가 남아 있으면 진행한다", () => {
  const config = { dailyCalls: 20, monthlyCalls: 500, dailyCostUsd: 1, monthlyCostUsd: 10 };
  const verdict = budgetDecision(
    { dailyCallsUsed: 0, monthlyCallsUsed: 0, dailyCostUsed: 99, monthlyCostUsed: 999 },
    5,
    config,
  );
  assert.equal(verdict.allowed, true, "비용 때문에 막히면 안 됩니다.");
  assert.ok(verdict.warning, "넘어선 사실은 알려 주어야 합니다.");
});

test("호출 횟수는 여전히 실제로 막는다", () => {
  // 비용을 경고로 내렸으니 지출을 묶는 것은 이제 이 값 하나뿐입니다.
  // 이것까지 느슨해지면 폭주를 세울 것이 아무것도 남지 않습니다.
  const config = { dailyCalls: 20, monthlyCalls: 500, dailyCostUsd: 999, monthlyCostUsd: 999 };
  const daily = budgetDecision(
    { dailyCallsUsed: 20, monthlyCallsUsed: 20, dailyCostUsed: 0, monthlyCostUsed: 0 },
    0,
    config,
  );
  assert.equal(daily.allowed, false);
  assert.match(daily.reason, /일일 Gemini 호출 상한/);

  const monthly = budgetDecision(
    { dailyCallsUsed: 0, monthlyCallsUsed: 500, dailyCostUsed: 0, monthlyCostUsed: 0 },
    0,
    config,
  );
  assert.equal(monthly.allowed, false);
  assert.match(monthly.reason, /월간 Gemini 호출 상한/);
});

test("예전처럼 막고 싶으면 block 으로 되돌릴 수 있다", () => {
  const config = { dailyCalls: 20, monthlyCalls: 500, dailyCostUsd: 1, monthlyCostUsd: 10 };
  const previous = process.env.GEMINI_COST_LIMIT_MODE;
  process.env.GEMINI_COST_LIMIT_MODE = "block";
  try {
    const verdict = budgetDecision(
      { dailyCallsUsed: 0, monthlyCallsUsed: 0, dailyCostUsed: 0, monthlyCostUsed: 999 },
      1,
      config,
    );
    assert.equal(verdict.allowed, false);
    assert.match(verdict.reason, /월간 Gemini 비용 상한/);
  } finally {
    if (previous === undefined) delete process.env.GEMINI_COST_LIMIT_MODE;
    else process.env.GEMINI_COST_LIMIT_MODE = previous;
  }
});

test("기본 상한에서는 횟수가 비용보다 먼저 걸린다", () => {
  // 비용 상한은 폭주를 막는 마지막 방어선이지 매일 쓰는 브레이크가 아닙니다.
  // 한 달 194회를 쓰고 비용에 먼저 막혔던 일이 이 순서가 뒤집혔던 결과입니다.
  for (const key of [
    "GEMINI_DAILY_CALL_LIMIT", "GEMINI_MONTHLY_CALL_LIMIT",
    "GEMINI_DAILY_COST_LIMIT_USD", "GEMINI_MONTHLY_COST_LIMIT_USD",
    "GEMINI_INPUT_USD_PER_MILLION_TOKENS", "GEMINI_OUTPUT_USD_PER_MILLION_TOKENS",
  ]) delete process.env[key];

  const config = geminiBudgetConfig();
  const perCall = estimatedGeminiCostUsd(20_000, 6_000, config);

  // 횟수 상한을 꽉 채워 써도 비용 상한에 닿지 않아야 합니다.
  assert.ok(perCall * config.dailyCalls < config.dailyCostUsd,
    `하루 ${config.dailyCalls}회면 ${(perCall * config.dailyCalls).toFixed(2)}달러인데 상한이 ${config.dailyCostUsd}달러입니다.`);
  assert.ok(perCall * config.monthlyCalls < config.monthlyCostUsd,
    `한 달 ${config.monthlyCalls}회면 ${(perCall * config.monthlyCalls).toFixed(2)}달러인데 상한이 ${config.monthlyCostUsd}달러입니다.`);

  // 예전에 막혔던 지점(한 달 194회)은 이제 통과해야 합니다.
  const verdict = budgetDecision(
    { dailyCallsUsed: 0, monthlyCallsUsed: 194, dailyCostUsed: 0, monthlyCostUsed: perCall * 194 },
    perCall * 6,
    config,
    6,
  );
