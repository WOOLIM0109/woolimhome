...
/**
 * 디자인 블로그 정보형 글의 주제 유형과 실제 예시.
 *
 * 컨설팅 쪽에는 이 목록이 있는데(CONSULTING_INFORMATIONAL_TOPIC_TYPES)
 * 디자인에는 없었습니다. 그래서 주제군만 주고 결은 안 알려 주었고,
 * 나오는 제목이 「효과적인 정보 구조 설계 방법」처럼 아무도 검색하지 않는
 * 말이 되었습니다.
 *
 * 입구는 사람들이 실제로 치는 말이고, 깊이는 안에서 공식 자료로 채웁니다.
 * 그대로 베끼지 않고 결만 참고합니다.
 */
export const DESIGN_INFORMATIONAL_TOPIC_TYPES = [
  {
    type: "규격·사이즈가 헷갈리는 것",
    examples: [
      "리플렛 사이즈 총정리, 3단 접지와 2단 접지 규격 비교",
      "전단지 표준 규격 A4·A5 중 무엇으로 뽑아야 할까",
      "현수막 사이즈 정하는 법, 걸 자리부터 재고 시작하세요",
      "명함 규격 90×50이 기본인 이유와 예외를 두는 경우",
    ],
  },
  {
    type: "인쇄 넘기기 전에 막히는 것",
    examples: [
      "인쇄소에 넘기기 전 도련(bleed) 3mm 확인하는 방법",
      "RGB로 만든 파일을 인쇄했더니 색이 달라진 이유",
      "인쇄용 PDF 저장 설정, 글꼴 포함과 이미지 압축 확인하기",
      "종이 평량 뭘 골라야 할까, 100g과 200g의 실제 차이",
    ],
  },
  {
    type: "저작권 때문에 사고 나는 것",
    examples: [
      "회사 소개서에 쓴 무료 폰트, 상업용 사용 가능한지 확인하는 법",
      "공공누리 안심글꼴 받아 쓰는 방법과 허용 범위",
      "무료 이미지 사이트에서 받은 사진, 인쇄물에 써도 되나요",
    ],
  },
  {
    type: "문서를 만들 때 막히는 것",
    examples: [
      "회사소개서 목차 구성, 열 장 안에 넣어야 할 것",
      "제안서 표지에 꼭 들어가야 하는 항목 정리",
      "IR 자료 몇 장이 적당할까, 투자자가 실제로 보는 순서",
      "PPT 발표자료 글자 크기, 뒷자리에서 읽히는 최소 기준",
    ],
  },
  {
    type: "자주 나오는 용어 구분",
    examples: [
      "CMYK와 RGB 차이, 언제 무엇을 쓰는지",
      "해상도 dpi와 ppi는 무엇이 다른가",
      "별색(스팟컬러)과 4도 인쇄, 비용이 갈리는 지점",
    ],
  },
] as const;
 ...
 * 영문 자료를 옮길 때 쓸 말.
 *
 * 새로 넣은 출처(Adobe, Microsoft, PANTONE, W3C 등)는 전부 영문입니다.
 * 번역 자체는 되지만 전문 용어를 제멋대로 옮깁니다. bleed 를 「블리드」나
 * 「출혈」로 쓰면 아무도 검색하지 않습니다. 검색되는 말로 들어오게 하려고
 * 주제군을 다시 짰는데, 본문에서 용어가 어긋나면 그 노력이 그대로 사라집니다.
 */
export const DESIGN_TERM_GLOSSARY = [
  ["bleed", "도련"],
  ["trim size", "재단 규격"],
  ["gutter", "안쪽 여백"],
  ["margin", "여백"],
