...
test("막힌 이유에는 그 항목의 숫자가 함께 나온다", () => {
  ...
  assert.match(cost.reason, /월간 Gemini 비용 상한/);
  assert.match(cost.detail, /\$9\.99/);
  assert.match(cost.detail, /\$10\.00/);
  assert.match(cost.detail, /GEMINI_MONTHLY_COST_LIMIT_USD/);
  assert.doesNotMatch(cost.detail, /194/);
