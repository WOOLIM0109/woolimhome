...
import { MAX_DIAGRAM_CHARS, countDiagrams, diagramIssues, diagramsEnabled, stripDiagrams } from "./diagram.ts";
...
test("너무 큰 도식은 잡아낸다", () => {
  /*
   * 도식은 본문과 한 덩어리로 넘어옵니다. 너무 크면 글을 다 쓰기 전에 출력
   * 한도에 닿아 응답이 통째로 끊깁니다. 실제로 그렇게 한 번 죽었고, 화면에는
   * "Unterminated string in JSON" 이라는 알 수 없는 말만 떴습니다.
   */
  const huge = `<svg viewBox="0 0 10 10"><title>제목</title><desc>설명</desc><text>글자</text>`
    + `<rect x="0" y="0" width="1" height="1" fill="#ef762f"/>`.repeat(80)
    + `</svg>`;
  assert.ok(huge.length > MAX_DIAGRAM_CHARS);
  assert.match(diagramIssues(huge, huge).join(" "), /너무 큽니다/);
});

test("적당한 크기는 통과한다", () => {
  assert.deepEqual(diagramIssues(GOOD, sanitizeGeneratedHtml(GOOD)), []);
  assert.ok(GOOD.length < MAX_DIAGRAM_CHARS);
});
