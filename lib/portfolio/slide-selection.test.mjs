...
test("penalizes text-heavy pages when visual scores are otherwise equal", () => {
  const base = {
    diagramRichness: 80,
    visualQuality: 80,
    rarity: 70,
  ...
  const balanced = scoreSlideAssessment({ slideIndex: 0, ...base, textDensity: 35 });
  const textHeavy = scoreSlideAssessment({ slideIndex: 1, ...base, textDensity: 95 });
  assert.ok(balanced.totalScore > textHeavy.totalScore);
  assert.equal(balanced.totalScore - textHeavy.totalScore, 12.25);
});
...
test("keeps the 24 highest-ranked slides for the approved long A4 landscape suite", () => {
  const selectedSlides = Array.from({ length: 30 }, (_, slideIndex) => ({
    slideIndex,
    totalScore: slideIndex >= 24 ? 99 : 80 - slideIndex,
    components: { diagramRichness: 80, visualQuality: 80, rarity: 80, sectionDiversity: 80 },
    reason: "테스트",
    rank: slideIndex >= 24 ? slideIndex - 23 : slideIndex + 7,
  }));
  const indexes = shortMockupRankedIndexes({ selectedSlides }, 24);
  for (const highRanked of [24, 25, 26, 27, 28, 29]) assert.ok(indexes.includes(highRanked));
  for (const lowRanked of [18, 19, 20, 21, 22, 23]) assert.ok(!indexes.includes(lowRanked));
});
...
test("never reintroduces ineligible slides while replacing short-document choices", () => {
  const assessments = Array.from({ length: 10 }, (_, slideIndex) => ({
    slideIndex,
    diagramRichness: 100 - slideIndex,
    visualQuality: 100 - slideIndex,
    rarity: 100 - slideIndex,
    recommended: true,
    visualHash: ((BigInt(slideIndex + 1) * BigInt("11400714819323198485"))
      & ((BigInt(1) << BigInt(64)) - BigInt(1))).toString(16).padStart(16, "0"),
  }));
  const eligibleSlideIndexes = [3, 4, 5, 6, 7, 8, 9];
  const result = selectPortfolioSlides({ slideCount: 10, assessments, eligibleSlideIndexes });
  assert.equal(result.mode, "short");
  assert.equal(result.selectedSlideIndexes.length, 7);
  assert.ok(result.selectedSlideIndexes.every((index) => eligibleSlideIndexes.includes(index)));
});

test("returns too few complete long groups when eligible slides cannot meet the minimum", () => {
  const assessments = Array.from({ length: 24 }, (_, slideIndex) => ({
    slideIndex,
    diagramRichness: 80,
    visualQuality: 80,
    rarity: 80,
  ...
  const eligibleSlideIndexes = Array.from({ length: 17 }, (_, index) => index + 7);
  const result = selectPortfolioSlides({ slideCount: 24, assessments, eligibleSlideIndexes });
  assert.equal(result.mode, "long");
  assert.equal(result.selectedSlideIndexes.length, 12);
  assert.ok(result.selectedSlideIndexes.every((index) => eligibleSlideIndexes.includes(index)));
  assert.equal(buildSixGridGroups(result.selectedSlideIndexes).length, 0);
});

test("applies the exclusion filter in long mode", () => {
  const assessments = Array.from({ length: 30 }, (_, slideIndex) => ({
    slideIndex,
    diagramRichness: 90,
    visualQuality: 90,
    rarity: 90,
  ...
  const result = selectPortfolioSlides({
    slideCount: 30,
    assessments,
    excludedSlideIndexes: [0, 1, 2],
  });
  assert.equal(result.mode, "long");
  assert.ok(result.selectedSlideIndexes.every((index) => ![0, 1, 2].includes(index)));
});

test("keeps long-document grouping when exported slides fall below 20", () => {
  const slideCount = 19;
  const assessments = Array.from({ length: slideCount }, (_, slideIndex) => ({
    slideIndex,
    diagramRichness: 90,
    visualQuality: 90,
    rarity: 90,
  ...
  const result = selectPortfolioSlides({
    slideCount,
    assessments,
    eligibleSlideIndexes: Array.from({ length: 18 }, (_, index) => index + 1),
    modeOverride: "long",
  });
  assert.equal(result.mode, "long");
  assert.equal(result.selectedSlideIndexes.length, 18);
  assert.equal(buildSixGridGroups(result.selectedSlideIndexes).length, 3);
});
