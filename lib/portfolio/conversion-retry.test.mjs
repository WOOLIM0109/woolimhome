...
test("a locally inspected slide-geometry failure is explicitly retryable after worker fixes", () => {
  assert.equal(portfolioConversionRecoveryState({
    status: "failed",
    errorMessage: "INSUFFICIENT_USABLE_SLIDES: Selective redaction left 3 fully inspected usable slide(s)",
  }), "retryable");
  assert.equal(portfolioConversionRecoveryState({
    status: "failed",
    errorMessage: "SHAPE_GEOMETRY_INSPECTION_FAILED: PowerPoint returned a non-positive shape boundary.",
  }), "retryable");
});
