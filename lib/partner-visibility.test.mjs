...
test("강제승인 메모는 주소 형식을 판단하지 않고 글자 그대로 받는다", () => {
  assert.equal(
    normalizePartnerForceApprovalMemo("  정상 주소 여부 확인 필요: 아무 내용  "),
    "정상 주소 여부 확인 필요: 아무 내용",
  );
});

test("강제승인 메모는 빈 값과 지나치게 긴 값만 받지 않는다", () => {
  assert.equal(normalizePartnerForceApprovalMemo("   "), null);
  assert.equal(normalizePartnerForceApprovalMemo("가".repeat(2001)), null);
});
...
test("관리자의 그대로 전달 승인은 외주 노출 문체 검사에도 적용한다", () => {
  const item = {
    channel: "naver_consulting",
    format: "informational",
    status: "approved",
    metadata: {
      generated: { bodyHtml: "<p>짧은 본문입니다.</p>", faq: [] },
      partnerReleaseOverride: { approvedAt: "2026-08-10T00:00:00.000Z" },
    },
  };

  assert.deepEqual(partnerEditorialPublicationIssues(item), []);
});

test("그대로 전달 승인이 없으면 외주 노출 문체 검사를 유지한다", () => {
  const issues = partnerEditorialPublicationIssues({
    format: "informational",
    metadata: { generated: { bodyHtml: "<p>짧은 본문입니다.</p>", faq: [] } },
  });
