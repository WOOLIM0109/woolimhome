...
- PPT는 PowerPoint와 설치 글꼴로 최대 24장의 대표 슬라이드를 PNG로 만듭니다.
- PDF는 `pdftoppm`으로 최대 100페이지를 렌더링한 뒤 최대 24장을 선택합니다.
...
새 버전의 이 폴더에서 아래 명령을 다시 실행하면 설정과 비밀키를 바꾸지 않고 워커 파일과 예약 작업만 갱신합니다.
