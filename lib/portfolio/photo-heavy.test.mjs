...
/** 큰 사진 한 장이 지면을 덮은 장표. 가리면 화면 절반이 뿌옇게 남습니다. */
const 큰사진장표 = {
  slideIndex: 4,
  regions: [
    region("embedded_photo", 0.05, 0.15, 0.9, 0.6),
    region("small_text", 0.05, 0.85, 0.3, 0.02),
  ],
};

test("큰 사진 한 장으로 꽉 찬 장표도 뺀다", () => {
  // 조각 세 개 규칙만 두었을 때 그대로 통과하던 경우입니다.
  assert.ok(photoCoverage(큰사진장표.regions) >= 0.45);
  assert.equal(isPhotoHeavySlide(큰사진장표), true);
});
