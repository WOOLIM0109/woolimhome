...
import { breadcrumbSchema, itemListSchema, newsArticleSchema } from "@/lib/schema";
    ...
    <>
      <JsonLd
        data={[
          ...
          itemListSchema("울림컴퍼니 소식/언론보도", news.map((item) => ({ title: item.title, description: item.summary, href: "/news" }))),
          newsArticleSchema(),
        ]}
      />
        ...
        <div className="mx-auto grid max-w-7xl gap-4 px-5 py-16 lg:px-8 lg:py-20">
          {news.map((item) => (
            <article key={item.title} className="card p-7 lg:flex-row lg:items-center lg:justify-between">
              ...
              <a href={item.href} target="_blank" rel="noreferrer" className="mt-5 inline-flex h-11 shrink-0 items-center gap-2 rounded-xl border border-[var(--line)] px-5 text-sm font-bold lg:mt-0">
