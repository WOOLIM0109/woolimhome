...
export const metadata: Metadata = {
  title: "성공사례 | 입찰/입점",
  description:
    "울림컴퍼니의 입찰제안서·입점 제안 주요 성과 — 공공조달 입찰 12억·10억 낙찰, 더현대·CJ온스타일·카카오 선물하기 입점 등.",
    ...
    <>
      <JsonLd
        data={[
          ...
          itemListSchema(
            "입찰/입점 성공사례",
            allItems.map((item) => ({ title: item.title, description: item.result, href: "/success/bidding-entry" })),
          ),
        ]}
      />
      <PageHero
      ...
      {pptCases.map((group, index) => {
        const Icon = group.icon;
        return (
          <section key={group.group} className={index % 2 ? "bg-[var(--surface-strong)]" : "bg-white"}>
