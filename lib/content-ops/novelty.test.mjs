...
test("수정 재생성은 과거 보류 문구 대신 짧은 보완 안내를 주제 계획에 기록한다", () => {
  const plan = {
  ...
  const revised = contentPlanForRevision(plan);
  assert.equal(revised.angle, "사용자의 최신 수정 요청에 따라 기존 글 보완");
  assert.doesNotMatch(revised.angle, /중복 검사 보류/);
