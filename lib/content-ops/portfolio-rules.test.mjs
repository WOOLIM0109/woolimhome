...
const CONVERSION = {
  status: "completed",
  result: { bucket: "slides", slidePaths: ["1.png", "2.png"] },
...
function portfolioMetadata(mode, boardSizes, redactionStatus = "verified") {
  ...
  return {
    ...
    portfolioMockup: {
      ...
      selectedSlideIndexes: bodyAssets.flatMap((asset) => asset.slideIndexes),
      redactionStatus,
    },
  };
}
...
test("accepts only the completed mockup job for the same source fingerprint", () => {
  ...
  assert.deepEqual(validatePortfolioSourceState(metadata, {
    status: "completed",
    result: { sourceFingerprint: SOURCE_FINGERPRINT },
  }, CONVERSION), []);
  ...
  assert.ok(validatePortfolioSourceState(metadata, {
    status: "completed",
    result: { sourceFingerprint: SOURCE_FINGERPRINT },
  }, { ...CONVERSION, updated_at: "2026-08-04T00:01:00.000Z" }).length > 0);
});
...
test("blocks portfolios produced by an older template or privacy rule version", () => {
  ...
  assert.ok(validatePortfolioSourceState(metadata, {
    status: "completed",
    result: { sourceFingerprint: SOURCE_FINGERPRINT },
  }, CONVERSION).some((issue) => issue.includes("규칙 버전")));
});
